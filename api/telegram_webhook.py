...
def do_publish(chat_id, draft_id):
    ...
    if candidate and candidate.get("status") == "published":
        reply(chat_id, "This candidate was already published (via a different draft). Not queuing.")
        return

    state_store.queue_draft(draft_id)

    slot = "tomorrow morning" if draft["draft_type"] == "post" else "tonight"
    reply(
        chat_id,
        f"Queued draft #{draft_id} ({draft['draft_type']}) -- will publish at the next {slot} slot.\n"
    ...
    try:
        ...
        if len(parts) < 2:
            reply(
                ...
                "/publish <draft_id> (queue for next scheduled slot)\n"
                "/publishnow <draft_id> (publish immediately)\n"
                "/discard <draft_id>",
            )
            return
