...
"infra/EMBA background" in every single draft, which made pillar 2 (tech +
business bridge) show up on nearly every post -- read as forced/rta rta.
Fixed by making that connection conditional ("only if the topic genuinely
has it") and by explicitly banning naming the framework/degree in the
output text itself. See config/positioning_strategy.md pillar 2 for the
full rationale.
"""
import os
